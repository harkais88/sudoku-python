...
class Game:
    def __init__(self):
        ...
        self.solution = np.empty(
            [9, 9]
        )  # Initializing an empty array that will be used for endgame
        # To avoid the chance that empty solution is equal to empty grid
        while np.all(np.equal(self.solution, self.game.grid)):
    ...
    def handle_event(self):
        ...
        if self.error_count == 5:
            ...
            if self.s_fail_f:
                self.s_fail.play()
                self.s_fail_f = not self.s_fail_f
        ...
        if np.all(np.equal(self.solution, self.game.grid)):
            ...
            self.screen.fill(self.background_fill)
            self.screen.blit(
                pygame.font.SysFont("Comic Sans MS", 30).render(win_txt[0], True, (0, 255, 0)),
            ...
            if self.s_success_f:
                self.s_success.play()
                self.s_success_f = not self.s_success_f
            pygame.display.update()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if (
                    self.start == False
                    and self.width // 2 - self.p < pos[0] < self.width // 2 + self.p
                ):
                ...
                if (
                    self.diff_flag == 1
                    and 2 * self.p < pos[0] < 2 * self.p + self.width - 4 * self.p
                ):
                    self.diff_flag = 0
                ...
                if (self.p < pos[0] < self.width - self.p) and (
                    self.p < pos[1] < self.width - self.p
                ):
            ...
            if event.type == pygame.KEYDOWN and self.diff_flag == 0 and self.main_flag == 0:
                ...
                if self.i != 10 and self.j != 10:
                    if event.key in self.event_map:
                        self.value = self.event_map[event.key]
                        # Input Event
    ...
    def draw(self):
        ...
        if self.start == False:
            ...
            self.screen.blit(
                start_text, (self.width // 2 - self.p + 2, self.width // 2 - self.p + 2)
            )
            ...
            self.screen.blit(
                quit_text, (self.width // 2 - self.p + 10, self.width // 2 + self.p + 2)
            )
            ...
            if self.main_flag == 1:
                ...
                rmv_title = pygame.draw.rect(
                    self.screen, self.background_fill, (0, 0, self.width, self.width // 2)
                )
                rmv_start = pygame.draw.rect(
                    self.screen,
                    self.background_fill,
                    (self.width // 2 - self.p, self.width // 2 - self.p, 2 * self.p, self.p),
                )
                rmv_quit = pygame.draw.rect(
                    self.screen,
                    self.background_fill,
                    (self.width // 2 - self.p, self.width // 2 + self.p, 2 * self.p, self.p),
                )
            ...
            if self.diff_flag == 1:
                ...
                pygame.draw.rect(
                    self.screen,
                    self.background_fill_alt,
                    (2 * self.p, 2 * self.p, self.width - 4 * self.p, self.p),
                    width=2,
                )
                ...
                pygame.draw.rect(
                    self.screen,
                    self.background_fill_alt,
                    (2 * self.p, 4 * self.p, self.width - 4 * self.p, self.p),
                    width=2,
                )
                med_text = diff_font.render("MEDIUM", True, diff_text_color)
                self.screen.blit(
                    med_text, (self.width // 2 - ((3 * self.p) // 2) - 6, 4 * self.p + 10)
                )
                ...
                pygame.draw.rect(
                    self.screen,
                    self.background_fill_alt,
                    (2 * self.p, 6 * self.p, self.width - 4 * self.p, self.p),
                    width=2,
                )
                ...
                pygame.draw.rect(
                    self.screen,
                    self.background_fill_alt,
                    (2 * self.p, 8 * self.p, self.width - 4 * self.p, self.p),
                    width=2,
                )
            ...
            if self.sel_diff != 0:
                ...
                self.screen.blit(
                    pygame.font.SysFont("Comic Sans Ms", 25).render(
                        "LOADING GAME", True, (0, 0, 0)
                    ),
            ...
            if self.pause_flag == True and self.diff_flag == 0:
                ...
                self.screen.blit(
                    pygame.font.SysFont("Comic Sans MS", 50).render("PAUSED!", True, (15, 163, 91)),
                    (self.width // 2 - (1.6 * self.p), self.p),
                )
                self.screen.blit(
                    pause_font.render(pause_txt[0], True, pause_color),
                    (self.width // 2 - (2.5 * self.p), 3 * self.p),
                )
                self.screen.blit(
                    pause_font.render(pause_txt[1], True, pause_color),
                    (self.width // 2 - (1.7 * self.p), 4.7 * self.p),
                )
                self.screen.blit(
                    pause_font.render(pause_txt[2], True, pause_color),
                    (self.width // 2 - (2.4 * self.p), 6.7 * self.p),
                )
                self.screen.blit(
                    pause_font.render(pause_txt[3], True, pause_color),
            ...
            if (
                ...
                and not np.all(np.equal(self.solution, self.game.grid))
                and self.pause_flag == False
            ):
                ...
                for i in range(10):
                    ...
                    pygame.draw.line(
                        self.screen,
                        (0, 0, 0),
                        (self.p + (self.p * i), self.p),
                        (self.p + (self.p * i), self.width - self.p),
                        width=line_width,
                    )
                    # Horizontal Line
                    pygame.draw.line(
                        self.screen,
                        (0, 0, 0),
                        (self.p, self.p + (self.p * i)),
                        (self.width - self.p, self.p + (self.p * i)),
                        width=line_width,
                    )
                ...
                # Timer
                time = num_font.render(
                    f"TIME: {(self.frame_count//60)//60}:{(self.frame_count//60)%60}",
                    True,
                    ori_num_color,
                )
                if not self.pause_flag:
                    self.frame_count += 1
                time_blank = pygame.draw.rect(
                    self.screen,
                    self.background_fill,
                ...
                # Error Counter
                if self.error_count > 0:
                    error_block = num_font.render(
                        f"MISTAKES: {self.error_count}/5", True, ori_num_color
                    )
                    error_blank = pygame.draw.rect(
                        self.screen, self.background_fill, (15, 5, 4.3 * self.p, self.p // 2 + 10)
                    )
                ...
                if self.sel_flag == 1:
                    ...
                    if (
                        self.sel_i != 10 and self.sel_j != 10
                    ):  # This is needed for the first selection cell
                        rmv_sel_cell = pygame.draw.rect(
                            self.screen,
                            self.background_fill,
                        ...
                        if self.solution[self.sel_i - 1][self.sel_j - 1] != 0:
                            if (
                                self.solution[self.sel_i - 1][self.sel_j - 1]
                                != self.game.grid[self.sel_i - 1][self.sel_j - 1]
                            ):
                                prev_sel_cell_val = num_font.render(
                                    str(self.solution[self.sel_i - 1][self.sel_j - 1]),
                                    True,
                                    err_color,
                                )
                            else:
                            ...
                            self.screen.blit(
                                prev_sel_cell_val,
                                (self.sel_j * self.p + 20, self.sel_i * self.p + 5),
                            )
                    ...
                    if self.solution[self.i - 1][self.j - 1] != 0:
                        selected_cell_val = num_font.render(
                            str(self.solution[self.i - 1][self.j - 1]), True, sel_num_color
                        )
                        self.screen.blit(
                            selected_cell_val, (self.j * self.p + 20, self.i * self.p + 5)
                        )
                ...
                for i in range(9):
                    for j in range(9):
                        ...
                        if self.game.puzzle[i][j] != 0:
                            puzzle_num = num_font.render(
                                str(self.game.puzzle[i][j]), True, ori_num_color
                            )
                            self.screen.blit(
                                puzzle_num, ((j + 1) * self.p + 20, (i + 1) * self.p + 5)
                            )
                        # Rendering Puzzle after Inputs
                        if self.game.puzzle[i][j] == 0 and self.solution[i][j] != 0:
                            if self.solution[i][j] != self.game.grid[i][j]:
                                puzzle_num = num_font.render(
                                    str(self.solution[i][j]), True, err_color
                                )
                            else:
                                puzzle_num = num_font.render(
                                    str(self.solution[i][j]), True, sol_num_color
                                )
                            self.screen.blit(
                                puzzle_num, ((j + 1) * self.p + 20, (i + 1) * self.p + 5)
                            )
