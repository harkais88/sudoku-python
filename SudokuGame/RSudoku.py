...
class sudoku:
    ...
    def diffSet(choice: int):
        ...
        diffMap = {
            1: np.random.randint(35, 46),
            2: np.random.randint(31, 35),
            3: np.random.randint(25, 31),
    ...
    def solver(self, table, row=0, col=0):
        ...
        for i in np.random.choice(np.arange(1, 10), 9, replace=False):
            if self.checkSafe(table, row, col, i):
                table[row][col] = i
                if self.solver(table, row, col + 1):
                    return True
                table[row][col] = 0
    ...
    def uniquePuzzle(self):
        ...
        while True:
            ...
            while counter < 20:
                ...
                if not np.all(np.equal(t_puzzle, self.grid)):
                    break
                counter += 1
